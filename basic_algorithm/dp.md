...
/*思路：根据股票买卖的动态规划感觉：
    二刷：20210627 动态规划
状态：[房屋i][是否偷窃],以i为结尾的房屋的最大偷窃收益
选择： dp[i][0] = Math.max(dp[i - 1][0],dp[i - 1][1]);
      dp[i][1] = dp[i - 1][0] + nums[i];//穷举计算所有状态
*/
    public int rob(int[] nums) {
        int[][] dp = new int[nums.length][2];//[房屋i][是否偷窃],以i为结尾的房屋的最大偷窃收益
        dp[0][0] = 0;
        dp[0][1] = nums[0];
...
```java
    ...
    //*方法二：动态规划从上往下：打家劫舍1：动态规划，先写出递归的，容易想到*/
	//1.状态：就是index索引，到哪儿了
    	////2.选择：当前点抢还是不抢
